...
import Starlight.util.abilityCards.FiredUpAbility;
import Starlight.util.abilityCards.FisticuffsAbility;
import Starlight.util.abilityCards.TemperanceAbility;
...
public class AbilityEffect extends AbstractGameEffect {
    ...
    public void update() {
        ...
        if (!AbstractDungeon.isScreenUp && !AbstractDungeon.gridSelectScreen.selectedCards.isEmpty() && AbstractDungeon.gridSelectScreen.forUpgrade) {
            ...
            if (card instanceof AbstractAbilityCard) {
                ((AbstractAbilityCard) card).upgradeAbility();
                AbilityManager.abilityPoints--;
                AbilityButton.flash();
            }
            if (AbstractDungeon.getCurrRoom() instanceof RestRoom) {
                RestRoom r = (RestRoom)AbstractDungeon.getCurrRoom();
                //((RestRoom)AbstractDungeon.getCurrRoom()).fadeIn();
                for (AbstractCampfireOption o : ReflectionHacks.<ArrayList<AbstractCampfireOption>>getPrivate(r.campfireUI, CampfireUI.class, "buttons")) {
                    if (o instanceof AbilityOption) {
                        ((AbilityOption) o).updateUsability();
                    }
                }
                if (AbilityManager.abilityPoints > 0) {
                    AbstractDungeon.effectsQueue.add(new AbilityEffect());
                } else {
                    r.campfireUI.reopen();
                }
            }
