...
public class SpellbookPanel {
    ...
    public float x;
    public float y;
    public static final float PAD_X = 130f * Settings.scale;
    public static final float PAD_Y = 130f * Settings.scale;
    public static final float CORR_X = 60f * Settings.scale;
    ...
    public void layoutBooks() {
        ...
        for (ClickableSpellbook book : books) {
            book.setPanel(this);
            book.move(x - (amtThisRow-1)/2f * PAD_X + index * PAD_X - CORR_X, y - row * PAD_Y);
            index++;
